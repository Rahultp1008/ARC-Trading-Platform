...
# Real imports — NOT inside TYPE_CHECKING — so SQLAlchemy registry can find them
...
class User(Base):
    ...
    # Leverage caps per product type
    ...
    broker: Mapped[Broker | None] = relationship(
        "Broker", back_populates="users", foreign_keys=[broker_id]
    )
